...
void vypisStats(int shortestPass, double countChar, double countPass,
                int difChars) {
...
int main(int argc, char *argv[]) {
  ...
  while (fgets(row, 102, stdin) != NULL) {
    ...
    if (bigger100) {
      fprintf(
          stderr,
          "Chyba vstupnych parametrov, maximalna dlzka hesla je 100 znakov\n");
      return 5;
    }
    ...
    if (stats) {
      ...
      // this section findes if the password contains new character whitch array
      // dosbt contain
      for (int i = 0; row[i] != '\n'; i++) {
